...
export const ACCOUNTS = [
  ...
  { id: 'inter', name: 'Inter', tail: '2810', type: 'checking' as const, balance: 12480.9 },
  { id: 'itau', name: 'Itaú poupança', tail: '0193', type: 'savings' as const, balance: 28900.0 },
]
...
export const RAW_TRANSACTIONS = [
  { id: 't1', date: 'hoje', name: 'iFood', categoryId: 'food', accountId: 'nu', amount: -38.9 },
  { id: 't2', date: 'hoje', name: 'Uber', categoryId: 'transport', accountId: 'nu', amount: -22.4 },
  {
    id: 't3',
    date: 'ontem',
    name: 'Mercado Pago',
  ...
  { id: 't9', date: '17 mai', name: 'Padaria', categoryId: 'food', accountId: 'nu', amount: -28.5 },
  {
    id: 't10',
    date: '15 mai',
    name: 'Amazon',
...
export const OUTGO = Math.abs(
  RAW_TRANSACTIONS.filter((t) => t.amount < 0).reduce((s, t) => s + t.amount, 0),
)
export const BALANCE = ACCOUNTS.reduce((s, a) => s + a.balance, 0)

export const BREAKDOWN: CategoryBreakdownItem[] = CATEGORIES_META.filter((c) => c.id !== 'salary')
  .map((c) => ({
    id: c.id,
    name: c.name,
    color: c.color,
    total: Math.abs(
      RAW_TRANSACTIONS.filter((t) => t.categoryId === c.id && t.amount < 0).reduce(
        (s, t) => s + t.amount,
        0,
      ),
    ),
